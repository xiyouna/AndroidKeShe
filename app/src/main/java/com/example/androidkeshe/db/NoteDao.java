...
import com.example.androidkeshe.NoteBean;
...
public class NoteDao {
    ...
    public List<NoteBean> queryNotesAll(String login_user, int mark) {
        ...
        NoteBean note;
        String             sql = "select * from note_data   order by note_id desc";
    ...
    public List<NoteBean> queryNotesAllByDate(String login_user, int mark, String year, String month, String day) {
        ...
        Cursor cursor = null;
        if (mark == 0) {//未完成备忘录
            sql = "select * from note_data where note_mark=0 and note_mark=time  order by note_id desc";
        } else if (mark == 1) {//已完成备忘录
            sql = "select * from note_data where   note_mark=1and createTime=time  order by note_id desc";
        } else {//所有备忘录
